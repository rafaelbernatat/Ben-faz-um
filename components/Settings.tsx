...
const Settings: React.FC<SettingsProps> = ({ data, onUpdate }) => {
  ...
  const pushToGithub = async () => {
    const repoPath = githubRepo.replace('https://github.com/', '').replace('.git', '').trim();
    ...
    try {
      ...
      let lastCommitSha = "";
      const refResp = await fetch(`https://api.github.com/repos/${repoPath}/git/refs/heads/main`, { headers });
      
      if (!refResp.ok) {
        // Repositório provavelmente vazio. Vamos inicializar com index.html
        setPushStatus('Repo vazio! Inicializando main...');
        const indexContent = await fetchFile('./index.html') || "<html><body>Site em construção</body></html>";
        
        const initResp = await fetch(`https://api.github.com/repos/${repoPath}/contents/index.html`, {
          method: 'PUT',
          headers,
          body: JSON.stringify({
        ...
        const retryRef = await fetch(`https://api.github.com/repos/${repoPath}/git/refs/heads/main`, { headers });
      ...
      const files = ['index.html', 'index.tsx', 'App.tsx', 'types.ts', 'constants.ts', 'metadata.json'];
      const components = ['Dashboard.tsx', 'BudgetManager.tsx', 'GuestList.tsx', 'VendorList.tsx', 'Settings.tsx', 'BottomNav.tsx'];
      const treeData = [];

      for (const file of files) {
        const content = await fetchFile(`./${file}`);
        if (content) treeData.push({ path: file, mode: '100644', type: 'blob', content });
      }
      for (const comp of components) {
        const content = await fetchFile(`./components/${comp}`);
        if (content) treeData.push({ path: `components/${comp}`, mode: '100644', type: 'blob', content });
      }

      // 3. Criar a Tree
      setPushStatus('Enviando arquivos...');
      const treeResp = await fetch(`https://api.github.com/repos/${repoPath}/git/trees`, {
        method: 'POST',
        headers,
      ...
      const commitResp = await fetch(`https://api.github.com/repos/${repoPath}/git/commits`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
      ...
      await fetch(`https://api.github.com/repos/${repoPath}/git/refs/heads/main`, {
        method: 'PATCH',
        headers,
    ...
    <div className="space-y-6 pb-12 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="bg-slate-900 p-6 rounded-3xl shadow-xl text-white overflow-hidden relative">
        ...
        <div className="flex items-center gap-3 mb-6 relative z-10">
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs">O repositório pode estar vazio</p>
          </div>
        </div>
        
        <div className="space-y-4 relative z-10">
          <div>
            <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1 ml-1">Token de Acesso</label>
          ...
          <div>
            <label className="block text-[10px] font-bold text-slate-500 uppercase mb-1 ml-1">Repositório</label>
            <input 
              type="text"
          ...
          >
            ...
            {pushStatus && <span className="text-[10px] font-normal text-white/50">{pushStatus}</span>}
          </button>

          {pushResult && (
            <div className={`text-xs text-center p-3 rounded-xl flex items-center justify-center gap-2 animate-in zoom-in-95 ${pushResult.success ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
              {pushResult.success ? <CheckCircle className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}
              {pushResult.msg}
            </div>
          )}
        </div>
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-5">
        ...
        <div className="space-y-4">
            <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 ml-1 uppercase">Nome da Celebração</label>
                <input 
                    type="text"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 ml-1 uppercase">Data</label>
                <input 
                    type="date"
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 mb-1 ml-1 uppercase">Teto de Gastos</label>
                <input 
                    type="number"
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-4">
        ...
        <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl flex gap-3">
          ...
          <p className="text-[11px] text-blue-800 leading-tight">
            Você e sua esposa estão conectados ao mesmo banco de dados. Alterações aparecem em ambos os aparelhos automaticamente.
          </p>
        </div>
        <div className="flex items-center gap-2 p-3 bg-slate-50 rounded-xl border border-slate-100">
            <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
            <span className="text-[10px] text-slate-500 font-medium truncate">{firebaseUrl}</span>
        </div>
      </div>

      <div className="text-center pt-8">
        <button 
          onClick={() => { if(confirm("Isso apagará apenas os dados salvos NESTE aparelho e recarregará da nuvem. Continuar?")) { localStorage.clear(); window.location.reload(); } }}
